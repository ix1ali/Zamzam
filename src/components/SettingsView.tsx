  ...
  return (
    <div style={{ padding: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <div>
          <h1 style={{ fontSize: '20px', fontWeight: 800, margin: 0 }}>{t('navSettings', lang)}</h1>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>{currentUser?.name || ''}</div>
        </div>
        <button onClick={handleLogout} style={{
          background: 'var(--danger-light)', color: 'var(--danger)', border: '1px solid var(--danger)',
          borderRadius: '12px', padding: '8px 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer',
      ...
      {/* Section Tabs */}
      <div style={{ display: 'flex', gap: '4px', marginBottom: '16px', background: 'var(--bg)', borderRadius: '14px', padding: '4px' }}>
        {sectionTabs.map(tab => (
      ...
      {activeSection === 'general' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', animation: 'fadeInUp 0.3s ease-out' }}>
          {/* Theme */}
          <div style={{ ...card, padding: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '12px' }}>{t('theme', lang)}</div>
            <div style={{ display: 'flex', gap: '8px' }}>
              {[
                ...
                <button key={th.id} onClick={() => handleThemeChange(th.id)} style={{
                  ...
                  fontWeight: theme === th.id ? 700 : 500, fontSize: '11px',
                  transition: 'all 0.2s ease',
                  boxShadow: theme === th.id ? 'var(--shadow-sm)' : 'none',
                }}>
                  <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d={th.icon} /></svg>
                  {th.label}
                </button>
              ))}
            </div>
          ...
          {/* Property Info */}
          <div style={{ ...card, padding: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '12px' }}>{t('propertyInfo', lang)}</div>
          ...
          {/* Quick Stats */}
          <div style={{ ...card, padding: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '12px' }}>{t('quickStats', lang)}</div>
          ...
          {/* About */}
          <div style={{ ...card, padding: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '12px' }}>{t('aboutApp', lang)}</div>
      ...
      {activeSection === 'audit' && (
        <div style={{ animation: 'fadeInUp 0.3s ease-out' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: 500 }}>{auditLog.length} {t('records', lang)}</span>
            {auditLog.length > 0 && (
              <button onClick={() => { if (confirm('مسح سجل النشاط؟')) { clearAuditLog(); reload(); showToast('تم مسح السجل'); } }} style={{
                background: 'var(--danger-light)', border: '1px solid var(--danger)', borderRadius: '10px',
                padding: '6px 12px', fontSize: '11px', color: 'var(--danger)', cursor: 'pointer', fontWeight: 600,
              }}>{t('clearAll', lang)}</button>
            )}
          </div>

          {auditLog.length === 0 ? (
            <div style={{ ...card, textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)', fontSize: '14px' }}>{t('noRecords', lang)}</div>
          ) : (
            <div style={{ ...card, overflow: 'hidden' }}>
              {auditLog.slice(0, 50).map(log => {
                const dateLocale = 'ar-KW';
                return (
                  <div key={log.id} style={{ padding: '10px 14px', borderBottom: '1px solid var(--border-light)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '12px' }}>
                          <span style={{
                            display: 'inline-block', padding: '2px 7px', borderRadius: '6px', fontSize: '10px', fontWeight: 700,
                            marginInlineEnd: '5px',
                        ...
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '3px' }}>{log.details}</div>
                      </div>
      ...
      {activeSection === 'users' && (
        <div style={{ animation: 'fadeInUp 0.3s ease-out' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: 500 }}>{users.length} {t('user', lang)}</span>
          ...
          <div style={{ ...card, overflow: 'hidden' }}>
            {users.map(u => (
              <div key={u.id} style={{ padding: '12px 14px', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{
                    width: '36px', height: '36px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: u.role === 'admin' ? 'var(--gradient-primary)' : 'var(--bg)',
      ...
      {activeSection === 'data' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', animation: 'fadeInUp 0.3s ease-out' }}>
          <div style={{ ...card, padding: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '10px' }}>{t('backup', lang)}</div>
            <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '12px', lineHeight: 1.6 }}>{t('backupDesc', lang)}</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={handleExportData} style={{
                flex: 1, padding: '11px', borderRadius: '12px', border: 'none',
                background: 'var(--gradient-success)', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer',
              ...
              <button onClick={handleImportData} style={{
                flex: 1, padding: '11px', borderRadius: '12px', border: 'none',
                background: 'var(--gradient-primary)', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px',
                boxShadow: '0 2px 8px rgba(30,58,95,0.25)',
              }}>
          ...
          <div style={{ ...card, padding: '16px', borderColor: 'var(--danger)' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '10px', color: 'var(--danger)' }}>{t('dangerZone', lang)}</div>
            <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '12px', lineHeight: 1.6 }}>{t('dangerDesc', lang)}</p>
            <button onClick={() => {
          ...
          <div style={{ ...card, padding: '16px' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, marginBottom: '10px' }}>{t('dataSize', lang)}</div>
  ...
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <h2 style={{ fontSize: '17px', fontWeight: 800, margin: '0 0 16px' }}>{t('addUser', lang)}</h2>
